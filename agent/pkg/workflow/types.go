...
// StreamCallback is called with text chunks as they arrive during streaming.
// The callback receives each text delta as it's generated by the LLM.
type StreamCallback func(textDelta string)

// CompleteOptions holds options for LLM completion.
type CompleteOptions struct {
	CacheSystemPrompt bool           // Enable prompt caching for the system prompt
	OnTextDelta       StreamCallback // Optional callback for streaming text chunks
}
...
// WithStreaming enables streaming mode and calls the provided callback
// with each text delta as it arrives from the LLM. This reduces perceived
// latency by showing output immediately rather than waiting for the full response.
func WithStreaming(callback StreamCallback) CompleteOption {
	return func(o *CompleteOptions) {
		o.OnTextDelta = callback
	}
}
