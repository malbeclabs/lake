...
**Feeds (SKUs)** map a feed to a metro and its joinable multicast groups: `dz_feeds_current` has `pk`, `code`, `name`, `metro_pk` (joins `dz_metros_current.pk`), and `groups` (JSON array of multicast-group PKs). `dz_users_current.feed_pk` links a connected EdgeSeat user to its feed and is an empty string (`''`, not the base58 zero pubkey that `tenant_pk`/`device_pk` use) for non-EdgeSeat/unicast users, so filter unset feeds with `feed_pk != ''`; `dz_access_passes_current.feed_seats` is a JSON array of per-feed seat billing (`feed_pk`, `max_users`, `current_users`, `window_end`, `terminates_at`, ...) on EdgeSeat passes.
