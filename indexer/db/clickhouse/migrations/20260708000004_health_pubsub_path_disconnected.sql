-- +goose Up
--
-- A publisher-subscriber path is 'disconnected' when either endpoint's BGP
-- session is down (no session -> no (S,G)/OIF, expected not faulty).
-- Requires 20260708000001.
...
SELECT
    ...
    multiIf(
        p.p_bgp_status = 'down' OR s.s_bgp_status = 'down', 'disconnected',
        has(coalesce(pi.pi_iif_tunnel_ids, []), p.p_tunnel_id)
