...
ALTER TABLE dim_dz_devices_history
    ADD COLUMN IF NOT EXISTS unicast_users_count UInt16 DEFAULT 0 AFTER max_multicast_publishers,
    ADD COLUMN IF NOT EXISTS multicast_subscribers_count UInt16 DEFAULT 0 AFTER unicast_users_count,
    ADD COLUMN IF NOT EXISTS reserved_seats UInt16 DEFAULT 0 AFTER multicast_subscribers_count,
    ADD COLUMN IF NOT EXISTS multicast_publishers_count UInt16 DEFAULT 0 AFTER reserved_seats;
-- +goose StatementEnd
...
-- +goose StatementBegin
ALTER TABLE dim_dz_devices_history
    DROP COLUMN IF EXISTS unicast_users_count,
    DROP COLUMN IF EXISTS multicast_subscribers_count,
    DROP COLUMN IF EXISTS reserved_seats,
    DROP COLUMN IF EXISTS multicast_publishers_count;
-- +goose StatementEnd
