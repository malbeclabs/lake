...
// capturingHandler records emitted records so tests can assert on their attrs.
...
func TestDeployStartOptionsRestartAtomically(t *testing.T) {
	...
	require.Equal(t, "indexer-dz-ingest-mainnet-beta", opts.ID)
	require.Equal(t, "indexer-dz-ingest-mainnet-beta", opts.TaskQueue)
...
// TestStartDZIngestWorkflowStartsAndLogsItsRun simulates a redeploy: each start issues
...
func TestStartDZIngestWorkflowStartsAndLogsItsRun(t *testing.T) {
	tc := &mocks.Client{}
	...
	for range 2 {
		h := &capturingHandler{}
		run, err := startDZIngestWorkflow(context.Background(), tc, slog.New(h), "mainnet-beta")
		require.NoError(t, err)

		rec, ok := findRecord(h.records, "dzingest: workflow started")
