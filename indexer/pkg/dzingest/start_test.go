...
// TestStartDZIngestWorkflowStartsAndLogsItsRun simulates a redeploy: each start
...
func TestStartDZIngestWorkflowStartsAndLogsItsRun(t *testing.T) {
	...
	for range 2 {
		...
		run, err := startDZIngestWorkflow(context.Background(), tc, log, "mainnet-beta")
		require.NoError(t, err)

		rec, ok := recs.Find("dzingest: workflow started")
...
func TestWatchWorkflowReattachesOnFailure(t *testing.T) {
	...
	_, ok := recs.Find("dzingest: workflow interrupted, reattaching")
