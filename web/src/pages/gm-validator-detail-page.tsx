...
import { fetchGMValidator, type GMTimePoint } from '@/lib/api'
...
export function GMValidatorDetailPage() {
  ...
  const { data: detail, isLoading, error } = useQuery({
    queryKey: ['gm-validator', pubkey, range, probeType],
    queryFn: () => fetchGMValidator(pubkey!, range, probeType),
    enabled: !!pubkey,
  })

  useDocumentTitle(detail ? truncatePubkey(detail.validator_pubkey) : 'Validator')
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-muted-foreground">Failed to load validator</p>
          <Link to="/gm/validators" className="text-sm text-blue-500 hover:underline">Back to validators</Link>
        </div>
      </div>
    )
  }

  const stakeSol = detail.stake_lamports / 1e9

  return (
    <div className="flex-1 overflow-auto">
      <div className="max-w-[1200px] mx-auto px-6 py-6">
        {/* Back link */}
        <Link to="/gm/validators" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-medium font-mono">{truncatePubkey(detail.validator_pubkey, 12)}</h1>
            ...
            <div className="flex items-center gap-3 mt-1 text-sm">
              <span className="text-muted-foreground">Stake: {stakeSol.toLocaleString(undefined, { maximumFractionDigits: 0 })} SOL</span>
              {detail.dzd_metro && (
