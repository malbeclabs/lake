...
import { fetchGMValidators, fetchGMValidatorsSummary, type GMValidatorItem, type GMTimePoint } from '@/lib/api'
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-2 text-xs text-muted-foreground">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block w-3 h-0.5 rounded border-dashed" style={{ background: piColor, borderTop: `2px dashed ${piColor}`, height: 0 }} />
          Public Internet
        </span>
      </div>
...
function matchesFilter(item: GMValidatorItem, filter: ParsedFilter): boolean {
  const val = filter.value.toLowerCase()
  const field = filter.field?.toLowerCase()
  if (!field) {
    // Search across all text fields
    return (
      item.validator_pubkey.toLowerCase().includes(val) ||
  ...
  switch (field) {
    case 'pubkey': return item.validator_pubkey.toLowerCase().includes(val)
...
export function GMValidatorsPage() {
  const navigate = useNavigate()
  const [searchParams, setSearchParams] = useSearchParams()

  const range = searchParams.get('range') || '24h'
  const probeType = searchParams.get('probe_type') || 'icmp'
  const sortBy = searchParams.get('sort') || 'stake_lamports'
  ...
  const { data: listData, isLoading: listLoading } = useQuery({
    queryKey: ['gm-validators', range, probeType],
    queryFn: () => fetchGMValidators(range, probeType),
    refetchInterval: 60000,
  })

  const { data: summary, isLoading: summaryLoading } = useQuery({
    queryKey: ['gm-validators-summary', range, probeType],
    queryFn: () => fetchGMValidatorsSummary(range, probeType),
    refetchInterval: 60000,
  })
    ...
    <div className="flex-1 overflow-auto">
      <div className="max-w-[1400px] mx-auto px-6 py-6">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center rounded-md border border-border overflow-hidden text-sm">
                {PROBE_OPTIONS.map(opt => (
                  <button
                    key={opt.value}
                    onClick={() => setParam('probe_type', opt.value)}
                    className={`px-3 py-1.5 transition-colors ${probeType === opt.value ? 'bg-accent text-accent-foreground' : 'hover:bg-muted'}`}
                  >
                    {opt.label}
                  </button>
                ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <StatCard label="Total Validators" value={summary?.total_validators} format="number" />
        ...
        <div className="mb-4 relative">
          ...
          <input
            ...
            placeholder="Filter validators... (e.g. metro:Dallas, country:US)"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50 text-left">
                  <th className="px-4 py-3 font-medium cursor-pointer" onClick={() => toggleSort('validator_pubkey')}>Pubkey<SortIcon field="validator_pubkey" /></th>
                  <th className="px-4 py-3 font-medium cursor-pointer text-right" onClick={() => toggleSort('stake_lamports')}>Stake<SortIcon field="stake_lamports" /></th>
              ...
              <tbody>
                {loading && pageItems.length === 0 ? (
                  ...
                  <tr><td colSpan={9} className="px-4 py-12 text-center text-muted-foreground">No validators found</td></tr>
                ) : (
                  pageItems.map(v => (
                    <tr
                      key={v.validator_pubkey}
                      className="border-b border-border hover:bg-muted/30 cursor-pointer transition-colors"
                      onClick={(e) => handleRowClick(e, `/gm/validators/${v.validator_pubkey}`, navigate)}
                    >
                      <td className="px-4 py-2.5 font-mono text-xs">{truncatePubkey(v.validator_pubkey)}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums">{formatStake(v.stake_lamports)}</td>
                      <td className="px-4 py-2.5">{v.metro || '-'}</td>
                      <td className="px-4 py-2.5">{v.country || '-'}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums">{formatPct(v.dz_availability_pct)}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums">{formatPct(v.pi_availability_pct)}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums">{formatRtt(v.dz_rtt_ms)}</td>
                      <td className="px-4 py-2.5 text-right tabular-nums">{formatRtt(v.pi_rtt_ms)}</td>
                      <td className={`px-4 py-2.5 text-right tabular-nums ${v.rtt_delta_ms > 0 ? 'text-green-500' : v.rtt_delta_ms < 0 ? 'text-red-500' : ''}`}>
                        {v.rtt_delta_ms === 0 ? '-' : `${v.rtt_delta_ms > 0 ? '+' : ''}${v.rtt_delta_ms.toFixed(1)}`}
                      </td>
                    </tr>
                  ))
                )}
