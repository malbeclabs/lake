...
function getJitterValue(latency: Record<string, number>, side: 'a' | 'z', mode: AggMode): number {
  const field = `${side}_${mode}_jitter_us`
  return (latency as Record<string, number>)[field] ?? 0
}

export function LinkJitterChart({ data, className, loading }: LinkJitterChartProps) {
  ...
  // Colors
  const aColor = isDark ? '#a855f7' : '#9333ea'
  const zColor = isDark ? '#f97316' : '#ea580c'
  ...
  const seriesKeys = useMemo(() => {
    ...
    if (hasCommittedJitter) keys.push('committed')
    return keys
  }, [hasCommittedJitter])
  ...
  const { uPlotData, uPlotSeries } = useMemo(() => {
    ...
    const fromA = buckets.map((b) => {
      if (!b.latency) return null
      const v = getJitterValue(b.latency as unknown as Record<string, number>, 'a', aggMode)
      return v > 0 ? v / 1000 : null
    })
    const fromZ = buckets.map((b) => {
      if (!b.latency) return null
      const v = getJitterValue(b.latency as unknown as Record<string, number>, 'z', aggMode)
      return v > 0 ? v / 1000 : null
    })

    const series: uPlot.Series[] = [
      {},
      { label: 'fromA', stroke: aColor, width: 1.5, points: { show: false } },
      { label: 'fromZ', stroke: zColor, width: 1.5, points: { show: false } },
    ]

    const dataArrays: (number | null)[][] = [fromA, fromZ]
