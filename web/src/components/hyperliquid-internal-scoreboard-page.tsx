...
import {
  fetchHyperliquidInternalScoreboard,
  type HyperliquidInternalScoreboardResponse,
...
export function HyperliquidInternalScoreboardPage() {
  const [timeWindow, setTimeWindow] = useState<(typeof WINDOWS)[number]>('24h')
  const [data, setData] = useState<HyperliquidInternalScoreboardResponse | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [now, setNow] = useState(() => Date.now())

  const load = useCallback(async () => {
    try {
      setData(await fetchHyperliquidInternalScoreboard(timeWindow))
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-8">
        <PageHeader
          icon={Trophy}
          title="Hyperliquid Internal Scoreboard"
          subtitle={
