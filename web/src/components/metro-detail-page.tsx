  ...
  const devices = useMemo(() => {
    return [...rawDevices].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'users': cmp = a.current_users - b.current_users; break
        case 'unicast': {
          ...
          if (!effA) return 1
          if (!effB) return -1
          cmp = (effA - a.unicast_users) / effA - (effB - b.unicast_users) / effB
          break
        }
        case 'subscribers': {
          ...
          if (!effA) return 1
          if (!effB) return -1
          cmp = (effA - a.multicast_subscribers_count) / effA - (effB - b.multicast_subscribers_count) / effB
          break
        }
        case 'publishers': {
          ...
          if (!effA) return 1
          if (!effB) return -1
          cmp = (effA - a.multicast_publishers_count) / effA - (effB - b.multicast_publishers_count) / effB
          break
        }
