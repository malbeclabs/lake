  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={cn(
          "w-full sm:w-auto relative flex items-center gap-1.5 px-3 py-1.5 text-xs xxs:text-sm rounded-md border transition-colors",
          hasFilter
      ...
      >
        ...
        <ChevronDown
          className={cn(
            "absolute right-3 sm:static h-3.5 w-3.5 transition-transform",
            open && "rotate-180",
          )}
        />
      </button>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 w-full min-w-40 bg-popover border border-border rounded-md shadow-lg py-1 whitespace-nowrap">
...
export function PublisherCheckPage() {
  ...
  const handleFilterSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      setSearchParams((prev) => {
        const p = new URLSearchParams(prev);
        if (filterInput) p.set("q", filterInput);
        else p.delete("q");
        p.delete("page");
        return p;
      });
    ...
    (filter: string) => {
      setSearchParams((prev) => {
        ...
        const current = new Set(
          (p.get("filter") || "").split(",").filter(Boolean),
        );
        if (current.has(filter)) current.delete(filter);
        else current.add(filter);
        if (current.size === 0) p.delete("filter");
        else p.set("filter", [...current].join(","));
        p.delete("page");
        return p;
      });
    },
  ...
  const handleEpochs = useCallback(
    (n: number) => {
      setSearchParams((prev) => {
        const p = new URLSearchParams(prev);
        if (n === 2) p.delete("epochs");
        else p.set("epochs", String(n));
        p.delete("page");
        return p;
      });
    },
  ...
  const handleToggleBackups = useCallback(() => {
    setSearchParams((prev) => {
      const p = new URLSearchParams(prev);
      if (p.get("backups") === "true") p.delete("backups");
      else p.set("backups", "true");
      p.delete("page");
      return p;
    });
  }, [setSearchParams]);
  ...
  const handleSlots = useCallback(
    (n: number) => {
      setSearchParams((prev) => {
        const p = new URLSearchParams(prev);
        if (n === 500) p.delete("slots");
        else p.set("slots", String(n));
        p.delete("page");
        return p;
      });
    },
  ...
  const handleToggleClient = useCallback(
    (client: string) => {
      setSearchParams((prev) => {
        ...
        if (current.size === 0) p.delete("client");
        else p.set("client", [...current].join(","));
        p.delete("page");
        return p;
      });
    },
  ...
  const sortedPublishers = useMemo(() => {
    ...
    return [...data.publishers].sort((a, b) => {
      ...
      switch (sortField) {
        case "publishing":
          cmp = Number(isPublishing(a)) - Number(isPublishing(b));
          break;
        case "publisher_ip":
          cmp = a.publisher_ip.localeCompare(b.publisher_ip);
          break;
        case "client_ip":
          cmp = a.client_ip.localeCompare(b.client_ip);
          break;
        case "dz_user_pubkey":
          cmp = a.dz_user_pubkey.localeCompare(b.dz_user_pubkey);
          break;
        case "vote_pubkey":
          cmp = a.vote_pubkey.localeCompare(b.vote_pubkey);
          break;
        case "validator_name":
          cmp = a.validator_name.localeCompare(b.validator_name);
          break;
        case "activated_stake":
          cmp = Number(a.activated_stake) - Number(b.activated_stake);
          break;
        case "dz_device_code":
          cmp = a.dz_device_code.localeCompare(b.dz_device_code);
          break;
        case "dz_metro_code":
          cmp = a.dz_metro_code.localeCompare(b.dz_metro_code);
          break;
        case "publishing_leader_shreds":
          cmp =
            Number(a.publishing_leader_shreds) -
            Number(b.publishing_leader_shreds);
          break;
        case "publishing_retransmitted":
          cmp =
            Number(a.publishing_retransmitted) -
            Number(b.publishing_retransmitted);
          break;
        case "leader_slots":
          cmp = a.leader_slots - b.leader_slots;
          break;
  ...
  const searchFilteredPublishers = useMemo(() => {
    ...
    return sortedPublishers.filter(
      (pub) =>
  ...
  const nonBackupPublishers = useMemo(
    () =>
      showBackups
        ? searchFilteredPublishers
        : searchFilteredPublishers.filter((pub) => !pub.is_backup),
  ...
  const clientFilteredPublishers = useMemo(() => {
    ...
    if (selectedClients.size > 0) {
      result = result.filter((pub) =>
        selectedClients.has(pub.validator_client.toLowerCase()),
      );
    }
    if (versionFilter) {
      result = result.filter((pub) =>
        pub.validator_version.startsWith(versionFilter),
      );
    }
    return result;
  ...
  const filteredPublishers = useMemo(() => {
    ...
    return clientFilteredPublishers.filter((pub) => {
      if (activeFilters.has("retransmit") && pub.publishing_retransmitted)
        return true;
      if (activeFilters.has("leader") && pub.publishing_leader_shreds)
        return true;
      if (
        activeFilters.has("not-publishing") &&
        !pub.publishing_leader_shreds &&
        !pub.publishing_retransmitted
      )
        return true;
      return false;
    });
  ...
  const publishingStake = useMemo(
    () =>
      clientFilteredPublishers
        .filter((p) => p.publishing_leader_shreds)
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="mx-auto px-4 sm:px-8 py-8">
        ...
        />
        <div className="mb-4 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-x-6 gap-y-3 w-full justify-between">
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 md:shrink-0">
            <div className="rounded-md bg-muted px-3 py-1.5 text-xs xxs:text-sm text-center sm:text-left">
              ...
              {(data?.total_publisher_stake ?? 0) > 0 && (
                <span
                  ...
                  title={formatStakeExact(data?.total_publisher_stake ?? 0)}
                >
            ...
            <div className="rounded-md bg-muted px-3 py-1.5 text-xs xxs:text-sm text-center sm:text-left">
              ...
              {publishingStake > 0 && (
                <span
                  className="ml-1.5 text-muted-foreground"
                  title={formatStakeExact(publishingStake)}
                >
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2">
            <div className="flex flex-col xs:flex-row w-full sm:w-fit rounded-md border border-border text-xs xxs:text-sm my-2 sm:my-0">
              {(
                [
                  ["epoch", "Epoch"],
                  ["slots", "Recent Slots"],
                ...
                <button
                  ...
                  className={cn(
                    "flex-1 px-3 py-1.5 transition-colors text-center whitespace-nowrap",
                    activeTab === value
            ...
            {activeTab === "epoch" ? (
              <div className="flex flex-col xs:flex-row w-full sm:w-fit rounded-md border border-border text-xs xxs:text-sm mb-2 sm:mb-0">
                {(
                  [
                    [1, "Current Epoch"],
                    [2, "Last 2 Epochs"],
              ...
              <div className="flex flex-col xs:flex-row w-full sm:w-fit rounded-md border border-border text-xs xxs:text-sm">
            ...
            {(
              ...
              <button
                ...
                className={cn(
                  "w-full sm:w-auto px-3 py-1.5 text-xs xxs:text-sm rounded-full border transition-colors text-center",
            ...
            <button
              ...
              className={cn(
                "w-full sm:w-auto px-3 py-1.5 text-xs xxs:text-sm rounded-md border transition-colors text-center mt-2 sm:mt-0",
                showBackups
                  ? "bg-accent text-accent-foreground border-accent"
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") handleVersionSubmit(versionInput);
                }}
                onBlur={() => handleVersionSubmit(versionInput)}
                className="w-full sm:w-20 px-2 py-1.5 text-xs xxs:text-sm text-center border border-border rounded-md bg-background focus:outline-none focus:ring-1 focus:ring-accent"
              />
              {versionFilter && (
                <button
                  type="button"
        ...
        <div className="mb-6 rounded-lg bg-muted/50 px-4 py-3 text-xs xxs:text-sm text-muted-foreground">
          <div className="flex items-start gap-2">
            <Info className="h-4 w-4 mt-0.5 shrink-0 hidden sm:block" />
            <ul className="space-y-1">
              <li>
                <span className="font-medium text-foreground">
                  Publishing Leader Shreds
                </span>{" "}
                — Leader shreds have been sent by the validator in the selected
                epoch range.
              </li>
              <li>
                <span className="font-medium text-foreground">
                ...
                — No retransmit shreds have been sent by the validator.
                Retransmit shreds are undesirable.
              </li>
            </ul>
          </div>
        </div>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="text-sm text-left text-muted-foreground border-b border-border">
                  <th
                    className={thCenter}
                    onClick={() => handleSort("publishing")}
                  >
                    Healthy
                    <SortIcon field="publishing" />
                  </th>
                  <th
                    className={thClass}
                    onClick={() => handleSort("publisher_ip")}
                  >
                    Publisher IP
                    <SortIcon field="publisher_ip" />
                  </th>
                  <th
                    className={thClass}
                  ...
                  <th
                    className={thClass}
                    onClick={() => handleSort("dz_user_pubkey")}
                  >
                    DZ ID
                    <SortIcon field="dz_user_pubkey" />
                  </th>
                  <th
                    className={thClass}
                  ...
                  <th
                    className={thClass}
                    onClick={() => handleSort("dz_device_code")}
                  >
                    Device
                    <SortIcon field="dz_device_code" />
                  </th>
                  <th
                    className={thClass}
                    onClick={() => handleSort("dz_metro_code")}
                  >
                    Metro
                    <SortIcon field="dz_metro_code" />
                  </th>
                  <th
                    className={thCenter}
                    onClick={() => handleSort("publishing_leader_shreds")}
                  >
                    Publishing Leader Shreds
                    <SortIcon field="publishing_leader_shreds" />
                  </th>
                  <th
                    className={thCenter}
                    onClick={() => handleSort("publishing_retransmitted")}
                  >
                    No Retransmit Shreds
                  ...
                  >
                    Leader Slots
                    <SortIcon field="leader_slots" />
                  </th>
                  <th
                    className={thClass}
                    onClick={() => handleSort("validator_client")}
                  >
                    Validator Client
              ...
              <tbody>
                {pagedPublishers.length === 0 ? (
                  <tr>
                    ...
                    >
                      {activeFilter
                        ? "No publishers found for this filter"
                        : "No publishers found"}
                    </td>
                  </tr>
                ) : (
                  pagedPublishers.map((pub) => (
                    ...
                    >
                      <td className="px-4 py-3 text-center">
                        {pub.is_backup ? (
                          <span className="inline-block rounded bg-muted px-1.5 py-0.5 text-xs xxs:text-sm text-muted-foreground">
                            Backup
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3 font-mono text-sm">
                        {pub.dz_user_pubkey ? (
                          <button
                            ...
                            title="Click to copy"
                            onClick={() =>
                              navigator.clipboard.writeText(pub.dz_user_pubkey)
                            }
                          >
                      ...
                      <td className="px-4 py-3 font-mono text-sm">
                        {pub.vote_pubkey ? (
                          <button
                            ...
                            title="Click to copy"
                            onClick={() =>
                              navigator.clipboard.writeText(pub.vote_pubkey)
                            }
                          >
                      ...
                      <td
                        ...
                        title={pub.validator_name}
                      >
                        {pub.validator_name || "\u2014"}
                      </td>
                      <td
                        className="px-4 py-3 text-sm tabular-nums"
                        title={formatStakeExact(pub.activated_stake)}
                      >
                        {pub.activated_stake
                          ? formatStake(pub.activated_stake)
                          : "\u2014"}
                      </td>
