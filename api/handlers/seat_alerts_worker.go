...
const seatAlertSweepInterval = 30 * time.Minute

// A claim older than this is reclaimable. It is shorter than the sweep interval
// so that at each tick exactly one replica wins the claim (the previous tick's
// claim is definitely stale), while a second replica ticking seconds later sees
// a fresh claim and skips.
const seatAlertClaimStale = 20 * time.Minute
...
func (a *API) StartSeatAlertWorker(ctx context.Context, serverID string) {
	ticker := time.NewTicker(seatAlertSweepInterval)
	go func() {
		for {
			select {
			case <-ctx.Done():
				...
				return
			case <-ticker.C:
				if a.claimSeatAlertSweep(ctx, serverID) {
...
// The claim is stale-reclaimable after one interval so a crashed claimant does
// not block sweeps forever.
func (a *API) claimSeatAlertSweep(ctx context.Context, serverID string) bool {
	var claimed bool
	err := a.PgPool.QueryRow(ctx, `
		...
		RETURNING true`,
		serverID, seatAlertClaimStale.Seconds()).Scan(&claimed)
	if err != nil {
