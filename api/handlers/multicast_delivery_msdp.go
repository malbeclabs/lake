...
// queryMulticastDeliveryMSDPPeers reads pre-enriched MSDP peer sessions
// from enriched_ip_msdp_peers. The view joins to the local device and
// resolves the 172.16.x.x mesh-space peer_address back to a peer device
// pubkey + code via dz_device_interface_ips. See
// indexer/db/clickhouse/migrations/20260602000002_enriched_ip_msdp_views.sql.
...
// The pim_sa_cache view doesn't carry remote_address / accept_status (the
// underlying PIM-side dim doesn't either); the includeRemote flag swaps those
// SELECT columns out for empty literals to keep the same scan shape.
