...
// would fail the new ownership check on their own runs. Ownership is set
// synchronously before the background goroutine, so reading the session row
// right after StartWorkflow returns is deterministic.
