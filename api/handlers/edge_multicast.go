...
func (a *API) FetchEdgeMulticastData(ctx context.Context) (*EdgeMulticastResponse, error) {
	...
	// The access-path RTT, from the ledger rather than from telemetry. Same contract: a missing
	// table or a failed read costs the figure, never the page.
	bgpRtt, err := a.queryEdgeMulticastBGPRtt(ctx)
	if err != nil {
		slog.Warn("edge multicast bgp rtt unavailable", "error", err)
		bgpRtt = nil
	}
...
func buildEdgeMulticastGroup(g MulticastDeliveryGroup, m edgeMulticastMembership, r edgeMulticastRates, lh edgeMulticastLastHeard, lines []EdgeMulticastPublisher, sequence *EdgeMulticastSequenceHealth, observations edgeMulticastObservationStatsResult, bgpSessions map[edgeMulticastBGPKey]EdgeMulticastBGPSession, bgpRtt map[string]EdgeMulticastBGPRtt) EdgeMulticastGroup {
	...
	attachEdgeMulticastSequenceHealth(lines, out.Sequence, observations.recorderLoss, observations.recorderLossSimul, observations.recorderLossUnavailable)
	...
	for i := range lines {
		...
		if r, ok := bgpRtt[lines[i].UserPK]; ok {
			rtt := r
			lines[i].BGPRtt = &rtt
		}
